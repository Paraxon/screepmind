...
export class State<actor_t, result_t> {
	...
	public constructor(name: string, body?: DecisionMaker<actor_t, result_t>, entry?: DecisionMaker<actor_t, result_t>, exit?: DecisionMaker<actor_t, result_t>) {
		this.name = name;
		this.body = body;
		this.entry = entry;
		this.exit = exit;
...
export class StateMachine<actor_t extends Stateful<actor_t, result_t>, result_t> implements DecisionMaker<actor_t, result_t>
{
	...
	decide(actor: actor_t): Action<actor_t, result_t> | undefined {
		...
		if (!actor.getState(this) || !this.graph.contains(actor.getState(this)!))
		...
		Logger.log("decision", `${actor} is transitioning from ${transition.from.name} to ${transition.to.name}`, Verbosity.Debug);
